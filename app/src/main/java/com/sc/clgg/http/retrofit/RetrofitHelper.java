...
public class RetrofitHelper {
    ...
    public retrofit2.Call<Check> update(String gender, String nickName, String realName, String clientSign) {
     ...
     * 获取用户信息
     */
    public retrofit2.Call<User> getUserInfo(String userName, String password) {
        Map<String, String> params = new HashMap<>();
        ...
        return Retrofit(NetField.CLGG_SITE).create(RetrofitApi.class).userInfo(json);
    }
    /**
     * 登录
     */
     ...
     * 手机号码校验
     */
    public retrofit2.Call<StatusBean> phoneCheck(String phone) {
        return Retrofit(NetField.SSO_SITE).create(RetrofitApi.class).phoneCheck(phone);
    }
     ...
     * 注册
     */
    public retrofit2.Call<StatusBean> register(String userName, String password, String personalPhone) {
        Map<String, String> params = new HashMap<>();
        params.put("userName", userName);
        ...
        RequestBody json = RequestBody.create(MediaType.parse("application/json;charset=UTF-8"), new Gson().toJson(params));
        return Retrofit(NetField.SSO_SITE).create(RetrofitApi.class).appRegister(json);
    }
    ...
    public retrofit2.Call<Check> checkUser(String userid) {
        return Retrofit().create(RetrofitApi.class).checkUser(userid);
    }
    ...
    private String getDefaultUserId() {
        return new ConfigUtil().getUserid().equals("") ? ConstantValue.TEST_ACCOUNT : new ConfigUtil().getUserid();
    }
...
//        File file = new File("/storage/emulated/0/dcim/Camera/B.png");
