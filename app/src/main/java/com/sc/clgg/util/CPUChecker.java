...
public class CPUChecker {
    ...
    public static String getArchType(Context context) {
        if (getSystemProperty(CPU_ARCHITECTURE_KEY_64, "").length() > 0) {
            ...
            if (LOGENABLE) {
                Log.v(tag, "return cpu DEFAULT 32bit!");
            }
    ...
    private static String getSystemProperty(String key, String defaultValue) {
        ...
        try {
            ...
            if (LOGENABLE) {
                Log.v(tag, "getSystemProperty    " + "key = " + key + ", error = " + e.getMessage());
            }
        }

        if (LOGENABLE) {
            Log.v(tag, "getSystemProperty    " + key + " = " + value);
        }
        return value;
    }
        ...
        if (cpuInfo != null && cpuInfo.exists()) {
            ...
            try {
                ...
                if (line != null && line.length() > 0 && line.toLowerCase(Locale.US).contains("arch64")) {
                    if (LOGENABLE) {
                        Log.v(tag, "###############isCPUInfo64()" + PROC_CPU_INFO_PATH + " contains is arch64");
                    }
                    return true;
                } else {
                    if (LOGENABLE) {
                        Log.d(tag, "###############isCPUInfo64()" + PROC_CPU_INFO_PATH + " is not arch64");
                    }
                }
            } catch (Throwable t) {
                if (LOGENABLE) {
                    Log.d(tag, "###############isCPUInfo64()" + "read " + PROC_CPU_INFO_PATH + " error = " + t.toString());
                }
            } finally {
                try {
    ...
    private static boolean isLibc64() {
        ...
        if (libcFile != null && libcFile.exists()) {
            ...
            if (header != null && header[EI_CLASS] == ELFCLASS64) {
                if (LOGENABLE) {
                    Log.d(tag, "###############isLibc64()" + SYSTEM_LIB_C_PATH + " is 64bit");
                }
                return true;
            }
        }
        ...
        if (libcFile64 != null && libcFile64.exists()) {
            ...
            if (header != null && header[EI_CLASS] == ELFCLASS64) {
                if (LOGENABLE) {
                    Log.d(tag, "###############isLibc64()" + SYSTEM_LIB_C_PATH_64 + " is 64bit");
                }
                return true;
            }
        }
