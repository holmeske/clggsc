...
public class MaintenancePresenter {
    ...
    public MaintenancePresenter(CommonContact contact) {
        this.contact = contact;
    }

    public void loadData(int pageno, int pageSize) {
        HttpRequestHelper.getMaintenanceData("3", pageno, pageSize, new HttpCallBack() {
            @Override
            public void onStart() {
                super.onStart();
                if (contact != null) {
                    contact.onStartRequest();
                }
            }
                ...
                if (bean != null) {
                    if (bean.getPage() != null) {
                        if (bean.getPage().getList() != null) {
                            if (contact != null) {
                                contact.onSuccess(bean.getPage().getList());
                            }
                        }
                    }
                }
