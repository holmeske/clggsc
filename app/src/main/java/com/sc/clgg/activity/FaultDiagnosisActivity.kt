...
//            iv_scan.startAnimation(animation)
            ...
            if (mSelectVehicleAdapter?.selectedList!!.isNotEmpty()) {
                faults.clear()
                diagnosis.text = "诊断中..."
                diagnosis(mSelectVehicleAdapter?.selectedList!!.get(0).vinNumber!!, 0)
            }
        }
    ...
    private fun initSelectVehicleView() {
        ...
        tv_sure.onClick {
            ...
            if (mSelectVehicleAdapter?.selectedList!!.size < 4) {
                ...
                stringBuilder.append(mSelectVehicleAdapter?.selectedList!!.get(0).carNumber).append("、")
                        .append(mSelectVehicleAdapter?.selectedList!!.get(1).carNumber).append("、")
                        .append(mSelectVehicleAdapter?.selectedList!!.get(2).carNumber)
            ...
            override fun onResponse(call: Call<Vehicle>?, response: Response<Vehicle>?) {
                response?.body()?.vehicleInfoList?.let {
                    ...
                    if (it.size == 1) {
                        faults.clear()
                        diagnosis.text = "诊断中..."
                        tv_des.text = "已选择${it.get(0).vinNumber}等${1}辆车"
    ...
    private fun diagnosis(vin: String, position: Int) {
        ...
        var vehicle = mSelectVehicleAdapter?.selectedList!!.get(position)
        ...
        call.enqueue(object : Callback<Fault> {
            override fun onFailure(call: Call<Fault>?, t: Throwable?) {
                ...
                if (position < size - 1) {
                    diagnosis(mSelectVehicleAdapter?.selectedList!!.get(position + 1).vinNumber!!, position + 1)
                }
            }
